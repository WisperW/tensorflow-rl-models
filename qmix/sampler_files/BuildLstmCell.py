...
class LSTMCell():
    def __init__(self, lstm_dim, scope, keep_prob, trainable=True):
    ...
    def __call__(self, inputx, current_state, new_traj_start):
        with tf.variable_scope(self.scope):
            wx = tf.get_variable("wx", [inputx.shape[-1], self.lstm_dim*4], initializer=tf.orthogonal_initializer, trainable=self.trainable)
            wh = tf.get_variable("wh", [self.lstm_dim, self.lstm_dim*4], initializer=tf.orthogonal_initializer, trainable=self.trainable)
            b = tf.get_variable("b", [self.lstm_dim*4], initializer=tf.constant_initializer(0.0), trainable=self.trainable)
...
class DropoutLSTMCell():
    def __init__(self, lstm_dim, scope, keep_prob):
        self.lstm_dim = lstm_dim
        self.scope = scope
...
class LNLSTMCell():
    def __init__(self, lstm_dim, scope):
        self.lstm_dim = lstm_dim
        self.scope = scope
